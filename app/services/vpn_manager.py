...
async def list_profiles() -> list[VpnProfile]:
    async with get_db() as conn, conn.execute("SELECT * FROM vpn_profiles ORDER BY name") as cur:
...
async def get_profile(profile_id: str) -> VpnProfile | None:
    async with (
        get_db() as conn,
        conn.execute("SELECT * FROM vpn_profiles WHERE id = ?", (profile_id,)) as cur,
    ):
    ...
    async with get_db() as conn:
        await conn.execute(
            ...
            (
                ...
                name,
                description,
                protocol.value if hasattr(protocol, "value") else protocol,
...
async def update_profile(profile_id, **kwargs) -> VpnProfile | None:
    fields, values = [], []
    for k, v in kwargs.items():
        if k not in ALLOWED_VPN_FIELDS:
            continue
        if v is None and k != "customer_id":
            continue
        if k == "config":
...
def _extract_secrets(config: dict, protocol) -> dict:
    ...
    if p == "wireguard":
        if "private_key" in config:
            secrets["private_key"] = config["private_key"]
        for i, peer in enumerate(config.get("peers", [])):
            if "preshared_key" in peer:
                secrets[f"peer_{i}_psk"] = peer["preshared_key"]
        ...
        if "server_secret_hex" in config:
            secrets["server_secret_hex"] = config["server_secret_hex"]
    return secrets
...
def _strip_secrets(config: dict, protocol) -> dict:
    ...
    if "peers" in c:
        c["peers"] = [
            {kk: vv for kk, vv in peer.items() if kk != "preshared_key"} for peer in c["peers"]
        ]
    return c
