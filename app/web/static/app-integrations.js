...
var _navParentMap = {
  ...
  'ssh': 'tools', 'browser': 'tools', 'provision': 'tools',
};
...
showView = function(name) {
  ...
  else if (name === 'workshop') workshopLoad();
  else if (name === 'ai') {
    aiLoadCustomers();
...
// ── Workshop view — shared notes for live sessions ──────────────────────────
// Renders docs/workshop-plan.md as a reference panel and lets attendees
// capture wishlist, per-section discussion notes, and follow-up actions.
// State persists to DATA_DIR/workshop_notes.json via /api/workshop/notes.
// All authenticated users can read+write — this is a collaborative
// scratchpad for a live session, not audit material.

var _workshopState = { wishlist: [], discussion_notes: {}, followups: [] };
...
async function workshopLoad() {
  // 1. Render the workshop plan markdown from docs/workshop-plan[.<lang>].md
  //    — probe the UI-language variant first (e.g. workshop-plan.en.md on
  //    English UI), fall back to the canonical Norwegian file otherwise.
  var planEl = document.getElementById('workshop-plan-body');
  if (planEl) {
    var lang = (typeof _lang !== 'undefined' && _lang) ? _lang : 'no';
    var data = await _wikiProbeDoc('workshop-plan.' + lang + '.md');
    if (!data) data = await _wikiProbeDoc('workshop-plan.md');
    if (data && data.content) {
      _wikiRenderInto(planEl, data.content);
    }
  }

  // 2. Load shared notes
  try {
    var notes = await apiFetch('/api/workshop/notes');
    if (notes && typeof notes === 'object') {
      _workshopState = {
        wishlist: Array.isArray(notes.wishlist) ? notes.wishlist.slice() : [],
        discussion_notes: (notes.discussion_notes && typeof notes.discussion_notes === 'object') ? Object.assign({}, notes.discussion_notes) : {},
        followups: Array.isArray(notes.followups) ? notes.followups.slice() : [],
      };
    }
  } catch (_) {}
...
function _workshopRender() {
  _workshopRenderWishlist();
  _workshopRenderNotes();
  _workshopRenderFollowups();
}

function _workshopRenderWishlist() {
  var list = document.getElementById('workshop-wishlist-list');
  var count = document.getElementById('workshop-wishlist-count');
  if (!list) return;
  var items = _workshopState.wishlist || [];
  if (count) count.textContent = items.length ? '(' + items.length + ')' : '';
  if (!items.length) {
    list.innerHTML = '<li style="color:var(--text-muted);font-style:italic;padding:6px 0;">' + esc(t('lbl_workshop_no_wishes','Ingen ønsker lagt til ennå')) + '</li>';
    return;
  }
...
function _workshopRenderNotes() {
  var box = document.getElementById('workshop-notes-container');
  if (!box) return;
...
function _workshopRenderFollowups() {
  var box = document.getElementById('workshop-followups-list');
  var count = document.getElementById('workshop-followups-count');
  if (!box) return;
  var items = _workshopState.followups || [];
  var openCount = items.filter(function(f) { return !f.done; }).length;
  if (count) count.textContent = items.length ? '(' + openCount + '/' + items.length + ')' : '';
  if (!items.length) {
    box.innerHTML = '<div style="color:var(--text-muted);font-style:italic;padding:6px 0;">' + esc(t('lbl_workshop_no_followups','Ingen oppfølgingspunkter ennå')) + '</div>';
    return;
  }
  box.innerHTML = items.map(function(f, i) {
    var textStyle = f.done ? 'text-decoration:line-through;color:var(--text-muted);' : '';
    var due = f.due ? '<span style="color:var(--text-muted);font-size:11px;margin-left:8px;">' + esc(f.due) + '</span>' : '';
    var owner = f.owner ? '<span style="color:var(--blue);font-size:11px;margin-left:8px;">@' + esc(f.owner) + '</span>' : '';
...
function workshopAddWishlist() {
  var inp = document.getElementById('workshop-wishlist-input');
  if (!inp) return;
  var val = (inp.value || '').trim();
  if (!val) return;
  _workshopState.wishlist = (_workshopState.wishlist || []).concat([val]);
  inp.value = '';
  _workshopRenderWishlist();
  _workshopQueueSave();
}

function workshopRemoveWishlist(i) {
  if (!_workshopState.wishlist) return;
  _workshopState.wishlist.splice(i, 1);
  _workshopRenderWishlist();
  _workshopQueueSave();
}

function workshopNoteChanged(sectionId, text) {
  _workshopState.discussion_notes = _workshopState.discussion_notes || {};
  _workshopState.discussion_notes[sectionId] = text;
  _workshopQueueSave();
}

function workshopAddFollowup() {
  var text = (document.getElementById('workshop-followup-text').value || '').trim();
  var owner = (document.getElementById('workshop-followup-owner').value || '').trim();
  var due = (document.getElementById('workshop-followup-due').value || '').trim();
  if (!text) return;
  _workshopState.followups = (_workshopState.followups || []).concat([{
    id: _workshopNextLocalId--,
    text: text, owner: owner, due: due, done: false,
  }]);
  document.getElementById('workshop-followup-text').value = '';
  document.getElementById('workshop-followup-owner').value = '';
  document.getElementById('workshop-followup-due').value = '';
  _workshopRenderFollowups();
  _workshopQueueSave();
}

function workshopToggleFollowup(i, done) {
  if (!_workshopState.followups || !_workshopState.followups[i]) return;
  _workshopState.followups[i].done = !!done;
  _workshopRenderFollowups();
  _workshopQueueSave();
}

function workshopRemoveFollowup(i) {
  if (!_workshopState.followups) return;
  _workshopState.followups.splice(i, 1);
  _workshopRenderFollowups();
  _workshopQueueSave();
}

function _workshopQueueSave() {
  var status = document.getElementById('workshop-save-status');
  if (status) status.textContent = t('msg_workshop_saving','Lagrer...');
  clearTimeout(_workshopSaveTimer);
  _workshopSaveTimer = setTimeout(_workshopSaveNow, 600);
}

async function _workshopSaveNow() {
  try {
    var resp = await apiFetch('/api/workshop/notes', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(_workshopState),
    });
    if (resp && resp.data) {
      // Adopt server-assigned ids for new followups.
      _workshopState = {
        wishlist: resp.data.wishlist || [],
        discussion_notes: resp.data.discussion_notes || {},
    ...
    var status = document.getElementById('workshop-save-status');
    if (status) status.innerHTML = '<span style="color:var(--color-danger);">' + esc(t('err_save_failed','Lagring feilet')) + '</span>';
  }
}

function _workshopMarkSaved() {
  var status = document.getElementById('workshop-save-status');
  if (!status) return;
  var now = new Date();
  status.textContent = t('msg_workshop_saved','Lagret') + ' ' + now.toLocaleTimeString('nb-NO', {hour:'2-digit',minute:'2-digit',second:'2-digit'});
}
