...
@router.post("/open-private")
async def open_private_browser(request: Request):
    ...
    try:
        if sys.platform == "darwin":
            ...
            if shutil.which("open"):
                ...
                for chrome in chrome_paths:
                    if Path(chrome).exists():
                        await asyncio.get_event_loop().run_in_executor(None, lambda c=chrome: subprocess.Popen(["open", "-na", c, "--args", "--incognito", url]))
                        return {"ok": True, "browser": "Chrome (incognito)"}
                # Edge InPrivate
                edge = "/Applications/Microsoft Edge.app"
                if Path(edge).exists():
                    await asyncio.get_event_loop().run_in_executor(None, lambda: subprocess.Popen(["open", "-na", edge, "--args", "--inprivate", url]))
                    return {"ok": True, "browser": "Edge (InPrivate)"}
                # Firefox private
                firefox = "/Applications/Firefox.app"
                if Path(firefox).exists():
                    await asyncio.get_event_loop().run_in_executor(None, lambda: subprocess.Popen(["open", "-na", firefox, "--args", "--private-window", url]))
                    return {"ok": True, "browser": "Firefox (privat)"}
                # Safari private — no CLI flag, open normally as fallback
                await asyncio.get_event_loop().run_in_executor(None, lambda: subprocess.Popen(["open", url]))
                return {"ok": True, "browser": "Standard (privat ikke tilgjengelig)"}
            ...
            if Path(edge).exists():
                await asyncio.get_event_loop().run_in_executor(None, lambda: subprocess.Popen([edge, "--inprivate", url]))
                return {"ok": True, "browser": "Edge (InPrivate)"}
            if Path(chrome).exists():
                await asyncio.get_event_loop().run_in_executor(None, lambda: subprocess.Popen([chrome, "--incognito", url]))
                return {"ok": True, "browser": "Chrome (incognito)"}
            if Path(firefox).exists():
                await asyncio.get_event_loop().run_in_executor(None, lambda: subprocess.Popen([firefox, "--private-window", url]))
                return {"ok": True, "browser": "Firefox (privat)"}
            import os
            os.startfile(url)
            return {"ok": True, "browser": "Standard"}
        else:
            # Linux
            ...
            if firefox:
                await asyncio.get_event_loop().run_in_executor(None, lambda: subprocess.Popen([firefox, "--private-window", url]))
                return {"ok": True, "browser": "Firefox (privat)"}
            await asyncio.get_event_loop().run_in_executor(None, lambda: subprocess.Popen(["xdg-open", url]))
            return {"ok": True, "browser": "Standard"}
...
async def open_folder(body: dict = Body(default={})):
    import subprocess
    import sys
    ...
    folder = body.get("path") or (str(state.audit_out_dir) if state.audit_out_dir else None) or str(get_audit_dir())
    ...
    try:
        ...
        folder_path = audit_root

    # If the specific path doesn't exist, fall back to parent or audit dir
    if not folder_path.exists():
        parent = folder_path.parent
        folder = str(parent) if parent.exists() and parent.resolve().is_relative_to(audit_root) else str(audit_root)
    else:
...
async def email_send_report(request: Request):
    ...
    if not state.audit_out_dir or not state.audit_out_dir.exists():
        raise ValidationError(ui_t("err_no_audit_results", request))

    # Find PDF
    pdf_path = None
    for f in state.audit_out_dir.iterdir():
        if f.suffix == ".pdf":
            pdf_path = f
            break
    ...
    # Load metrics
    metrics = None
    metrics_path = state.audit_out_dir / "_audit_metrics.json"
    ...
    customer_name = state.audit_out_dir.parent.name.replace("_", " ")
    run_date = state.audit_out_dir.name
    ...
    try:
        ...
        raise IntegrationError("Kunne ikke sende e-post")
...
async def customer_summary_report(
    ...
    def _grade_color(g: str) -> str:
        return {"A": "#3fb950", "B": "#4d9fb5", "C": "#d29922", "D": "#f85149", "F": "#8b0000"}.get(g, "#888")

    def _check(ok: bool) -> str:
        return '<span style="color:#3fb950;font-weight:700;">&#10003;</span>' if ok else '<span style="color:#f85149;font-weight:700;">&#10007;</span>'
...
async def export_csv():
    ...
    if state.audit_out_dir:
        dir_customer = state.audit_out_dir.parent.name.replace("_", " ")
    ...
    writer.writerow(["MFA", "Uten MFA", ctx["mfa"]["no_mfa"], "Kritisk" if ctx["mfa"]["no_mfa"] > 0 else "OK"])
    ...
    for lic in ctx.get("licenses", []):
        ...
        writer.writerow(["Lisens", lic["part"], f"{lic['used']}/{lic['total']} ({lic['pct']:.0f}%)", status])
    ...
    csv_path = state.audit_out_dir / f"audit_export_{customer_name.replace(' ', '_')}.csv"
...
async def export_dashboard_excel(user: User = Depends(get_current_user)):
    ...
    headers = [
        "Customer", "Domain", "Risk Grade", "Risk Score",
        "MFA Coverage %", "Secure Score %", "Total Users",
        "Users Without MFA", "CA Policies", "Intune Compliance %",
        "Global Admins", "Last Audit Date", "Tags",
    ]
    ...
    for c in customers:
        ...
        if last_audit and len(last_audit) >= 13:
            try:
                fmt_date = f"{last_audit[8:10]}:{last_audit[11:13]} {last_audit[0:10].replace('-', '.')}"
        ...
        rows.append([
            ...
            round(metrics["intune_compliance_pct"], 1) if "intune_compliance_pct" in metrics else "",
...
async def generate_report(request: Request):
    if not state.audit_out_dir or not state.audit_results:
        raise ValidationError(ui_t("err_no_audit_results", request))
    ...
    try:
        ...
        raise IntegrationError("Rapportgenerering feilet")
    ...
    for customer_dir in audit_dir.iterdir():
        ...
        for run_dir in list(customer_dir.iterdir()):
            ...
            try:
                ...
                run_date = datetime.strptime(date_str, "%Y-%m-%d").replace(tzinfo=timezone.utc)
...
async def batch_summary_report(request: Request, user: User = Depends(get_current_user)):
    ...
    avg_score = round(sum(s.get("score", 0) for s in summaries if isinstance(s.get("score"), (int, float))) / len(summaries), 1) if summaries else 0
    avg_mfa = round(sum(s.get("mfa", 0) or 0 for s in summaries) / len(summaries), 1) if summaries else 0
...
<tbody>"""
    ...
    for s in summaries:
        ...
        mfa_color = "#3fb950" if (s["mfa"] or 0) >= 95 else "#d29922" if (s["mfa"] or 0) >= 80 else "#f85149"
        html += f"""<tr>
