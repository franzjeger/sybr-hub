...
## 1. Assessment library — "Assess" pillar (Fase B) — DONE

Delivered: a browsable **Assessment library** under Customers →
*Vurderingsbibliotek* (`app/web/static/app-assessments.js`, view
`view-assessments`, feature `assessments` at viewer role). Pick a customer, run
a framework against the latest audit, read named per-check results with `why`
and reason.

- Item 1 — measurable-path reference: `docs/baseline-context-paths.md`
  (regenerated by `scripts/gen_baseline_paths.py`) plus the guard test
  `tests/test_baseline_paths_are_measurable.py`, which fails any check whose
  `path` the audit never produces (`field_absent`/`incomparable`).
- Item 2 — `app/baselines/essential-8-l1.json` (8 checks: MFA, restrict admins,
  managed/compliant devices, backup; the four endpoint-only E8 strategies left
  out honestly).
- Item 3 — `app/baselines/cis-m365-foundations-subset.json` (9 checks, named as
  a subset — only the structured signals: MFA, CA, legacy auth, GA count, Secure
  Score, external sharing, device compliance).
- Item 4 — `app/baselines/nis2-hardening.json` (10 checks mapped to Article
  21(2) measures; scope-limited to what an audit measures).
- Item 5 — library UI modelled on the customer-card baseline panel; class-based
  to stay under the CSP budget, all strings in `ui_i18n.json`, wired in
  `test_assessments_view.py`.

Supporting change: `_parse_sharepoint_settings` now returns `sharing_known`
(tri-state, mirrors `legacy_auth_known`) so an external-sharing check can guard
against an unread setting instead of scoring it as a finding.
