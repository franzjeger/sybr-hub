...
@pytest.mark.asyncio
async def test_config_temp_is_removed_when_openvpn_subprocess_fails(tmp_path, monkeypatch):
    config_path = tmp_path / "private.ovpn"

    def write_private(content: str, _suffix: str):
        config_path.write_text(content)
        config_path.chmod(0o600)
        return config_path

    async def fail_to_start(*_args, **_kwargs):
        raise RuntimeError("openvpn3 failed")

    monkeypatch.setattr(azure, "_write_private", write_private)
    monkeypatch.setattr(shutil, "which", lambda _name: "/usr/bin/openvpn3")
    monkeypatch.setattr(azure.asyncio, "create_subprocess_exec", fail_to_start)

    with pytest.raises(RuntimeError):
        await azure.connect({"gateway_fqdn": "gw.example.com"}, "access-token")
