...
class FakeGraph:
    ...
    async def get(self, path, **kw):
        ...
        if path.startswith("users/"):
            uid = path.split("/")[1]
            drive = self.user_drives.get(uid)
            if drive is None:
                raise RuntimeError("404 no OneDrive provisioned")
            return drive
...
class TestTheScanReachesTheWholeTenant:
    async def test_every_site_drive_and_every_user_onedrive_is_visited(self, tmp_path):
        ...
        assert "users/u1/drive" in graph.calls, "the user's OneDrive was never located"
